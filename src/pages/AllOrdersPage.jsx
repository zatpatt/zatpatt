...
export default function AllOrdersPage() {
  ...
  const filteredOrders = orders
    ...
    .sort((a, b) => {
      if (sort === "price_high") return Number(b.total_amount) - Number(a.total_amount);
      if (sort === "price_low") return Number(a.total_amount) - Number(b.total_amount);
      return 0;
    });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-white">
      ...
      <header className="bg-orange-500 text-white py-4 px-6 shadow-lg flex items-center justify-center relative">
        <button
          onClick={() => navigate(-1)}
          className="absolute left-5 top-3 p-2 rounded-full bg-white shadow"
        >
      ...
      {/* ORDER DETAIL MODAL (SIMPLE + SAFE) */}
      <AnimatePresence>
        {selectedOrder && (
          <motion.div className="fixed inset-0 bg-black/50 flex items-end justify-center p-4">
            <motion.div className="bg-white w-full max-w-lg rounded-t-2xl p-5">
              <p className="font-semibold">{selectedOrder.order_code}</p>
              <p className="text-sm capitalize text-gray-500">
                Status: {selectedOrder.status}
              </p>
              <p className="font-bold mt-2">₹{selectedOrder.total_amount}</p>

              <button
                className="mt-4 bg-gray-200 w-full py-2 rounded-xl"
...
//   )
//   .sort((a, b) => {
//     if (sort === "price_high") return b.total_amount - a.total_amount;
//     if (sort === "price_low") return a.total_amount - b.total_amount;
//     return 0;
//   });
