...
import { getLeaderBoardApi } from "../services/leaderboardApi";
...
export default function LeaderboardPage() {
  ...
  const currentUser = "Sai Patil"; // 🔥 Replace with logged in user name
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      setTimeout(() => setConfettiRunning(false), 3000);
    };
  ...
  const maxPoints =
    users.length > 0 ? Math.max(...users.map((u) => u.points)) || 1 : 1;
  ...
  return (
    <div className="min-h-screen bg-[#fff9f4] relative pb-16">
      {/* Confetti */}
      {confettiRunning && (
        <Confetti
      ...
      <div className="p-6">
        {users.map((user, i) => (
          ...
          >
            <div className="flex items-center gap-4 w-full">
              <div className="relative w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center font-bold">
                {user.name[0]}
        ...
        {/* If current user not in top 3 */}
        {currentUserIndex >= 3 && (
          <div className="flex justify-between items-center p-4 rounded-2xl shadow-md bg-orange-50 mt-4">
            <span className="font-semibold">
              Your Rank: {currentUserIndex + 1}
            </span>
            <span className="font-bold text-orange-500">
              {users[currentUserIndex]?.points || 0} orders
            </span>
          </div>
        )}
      </div>
...
//   { key: "BackendOrders", label: "Live", icon: <Users size={20} /> }, // 👈 ADD THIS
